...
- **日报审验-自补（2026-09-05，①~⑥ ✅，⑦~ 未批待开工门）**：① 33_reviewer schema（051dc9c）② 注入 3 bug 修复（8958602）③ check_report_inputs（375dc63，单测 18/18）④ reviewer_agent 只读 dry-run（b3bd37b，单测 17/17）⑤ report_assembler 装配器（c0bbe32，单测 19/19）⑥ daily_review_gate 审验闭环 runner + cron 0073d213839b 接入（7bf5f8e，单测 12/12）——cron prompt step 3c 加审验闭环，开关 data/review_gate_config.json REVIEWER_ENABLED=false 灰度一期（审验日志照记正文不改）；gate 退出码：0 正常 / 2 fail_closed 转人工(开装配后) / 3 审验异常 degrade。**⑦~ 后续步（下会话候选）**：美股夜盘采集器 us_market_fetch（新浪映射已实测 int_dji/int_nasdaq/int_sp500/znb_VIX/gb_nvda 等；清单=指数4+tracking pool 美股；独立落 data/market/us_cache.json）→ Reviewer LLM 语义审验（active-repair）→ 灰度一周（9-07 起 22:00）后评估 REVIEWER_ENABLED=true。cron 改 prompt 命令：hermes cron edit 0073d213839b --prompt "$(cat <file>)"
