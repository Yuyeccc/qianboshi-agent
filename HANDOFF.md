...
- **日报审验-自补（2026-09-05，①~⑦ ✅，⑧~ 未批待评估）**：① 33_reviewer schema（051dc9c）② 注入 3 bug 修复（8958602）③ check_report_inputs（375dc63，单测 18/18）④ reviewer_agent 只读 dry-run（b3bd37b，单测 17/17）⑤ report_assembler 装配器（c0bbe32，单测 19/19）⑥ daily_review_gate 审验闭环 runner + cron 0073d213839b 接入（7bf5f8e，单测 12/12）——cron prompt step 3c 加审验闭环，开关 data/review_gate_config.json REVIEWER_ENABLED=false 灰度一期（审验日志照记正文不改）；gate 退出码：0 正常 / 2 fail_closed 转人工(开装配后) / 3 审验异常 degrade。**⑦ 美股夜盘采集器 ✅（fe1223d/8ab42a2，单测 24/24）**：scripts/us_market_fetch.py 新浪 hq.sinajs.cn 实时快照→data/market/us_cache.json（不入 git，.gitignore 已补 8ab42a2）；清单=pool.us_indexes 3+内置 ^VIX+pool.us_stocks 7，gb_ 昨收=串末字段勿回退 [28]，requests 须 Session trust_env=False；降级链 live→stale→missing 断网实测全通（exit 0 不崩）；9-07 周一 22:00 首跑=夏令时美东 10:00 盘中 live 灰度观察点。**⑧~ 后续候选（下会话）**：日报接入 us_cache 写美股段（动生产 8 章链路，单独立刀）or 美股夜盘消费评估 → Reviewer LLM 语义审验（active-repair）→ 灰度一周后评估 REVIEWER_ENABLED=true。cron 改 prompt 命令：hermes cron edit 0073d213839b --prompt "$(cat <file>)"
